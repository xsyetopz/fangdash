...
export const raceRouter = router({
  submitResult: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const otherAffected = placementChanges.filter((c) =>
        c.playerId !== playerRecord.id
      );
      ...
      for (const other of otherAffected) {
        ...
        const xpDelta = getPlacementBonus(other.newPlacement) -
        ...
        const racesWonDelta = (other.oldPlacement === 1 ? -1 : 0) +
      ...
      batchStatements.push(
        ctx.db.update(player).set(currentPlayerUpdate).where(
          eq(player.id, playerRecord.id),
        ),
      );
