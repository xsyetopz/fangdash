...
export const scoreRouter = router({
  submit: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const maxAllowedScore = ((input.duration / 1000) * SCORE_PER_SECOND +
      ...
      await ctx.db
        ...
        .set({
          ...
          totalObstaclesCleared:
            sql`${player.totalObstaclesCleared} + ${input.obstaclesCleared}`,
  ...
  leaderboard: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const diffFilter = difficulty
        ? sql` AND s2.difficulty = ${difficulty}`
        : sql``;
      const outerDiffFilter = difficulty
        ? sql` AND ${score.difficulty} = ${difficulty}`
        : sql``;
      // When filtering by a specific mod bitmask:
      //   - mods === 0 → exact match (no mods only)
      //   - mods > 0  → containment match: score must have AT LEAST the selected mods
      // When showing "All" (mods undefined), exclude scores with non-ready mods.
      const readyMask = READY_MODS_MASK;
      const modsFilter = mods === undefined
        ? sql` AND (s2.mods & ~${readyMask}) = 0`
        : mods === 0
        ? sql` AND s2.mods = 0`
        : sql` AND (s2.mods & ${mods}) = ${mods}`;
      const outerModsFilter = mods === undefined
        ? sql` AND (${score.mods} & ~${readyMask}) = 0`
        : mods === 0
        ? sql` AND ${score.mods} = 0`
        : sql` AND (${score.mods} & ${mods}) = ${mods}`;
      const cutoffFilter = cutoffTs !== null
        ? sql` AND s2.created_at >= ${cutoffTs}`
        : sql``;
      const outerCutoffFilter = cutoffTs !== null
        ? sql` AND ${score.createdAt} >= ${cutoffTs}`
        : sql``;
  ...
  myScores: protectedProcedure.query(async ({ ctx }) => {
    ...
    return ctx.db
      ...
      .from(score)
      .where(
        sql`${score.playerId} = ${playerRecord.id} AND ${score.cheated} = 0`,
      )
