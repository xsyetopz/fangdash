...
export async function checkAchievements(
  ...
  // Get player stats
  const playerRecord = await db.select().from(player).where(
  ...
  const stats: PlayerStats = {
    highestScore: Math.max(
      maxStats?.highestScore ?? 0,
      latestScore?.score ?? 0,
    ),
    highestDistance: Math.max(
      maxStats?.highestDistance ?? 0,
    ...
    longestCleanRun: Math.max(
      maxStats?.highestCleanRun ?? 0,
      latestScore?.longestCleanRun ?? 0,
    ),
