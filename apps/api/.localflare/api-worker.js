...
function isQueue(binding) {
	return (
		binding !== null && typeof binding === "object" && "send" in binding && "sendBatch" in binding
	);
}
...
function createD1Routes() {
	...
	app2.put("/:binding/tables/:table/rows/:rowId", async (c) => {
		...
		try {
			...
			const updateEntries = Object.entries(data).filter(([k]) => !primaryKeys.includes(k));
	...
	app2.post("/:binding/tables/:table/bulk-delete", async (c) => {
		...
		try {
			...
			for (const rowId of rowIds) {
				const { clause: whereClause, values: whereValues } = buildPrimaryKeyWhere(
					primaryKeys,
					rowId,
				);
				const sql = `DELETE FROM ${escapeIdentifier(tableName)} WHERE ${whereClause}`;
...
function createLogsRoutes() {
	...
	app2.post("/", async (c) => {
		try {
			...
			logStore.log(body.level || "info", body.message, body.data, body.source || "system");
			return c.json({ success: true });
		} catch {
			...
			return c.json(
				{
					...
					message:
						"No API credentials found. Please configure your Cloudflare API credentials in Settings, or set CF_ACCOUNT_ID and CF_API_TOKEN environment variables.",
					datasets: [],
				},
				400,
			);
			...
			return c.json(
				{
					error: "Missing configuration",
					message:
						"No API credentials found. Please configure your Cloudflare API credentials in Settings.",
					columns: [],
				},
				400,
			);
			...
			if (params) {
				Object.entries(params).forEach(([key, value]) => {
					...
					if (typeof value === "string") {
						const isIntervalValue = /^'\d+'\s+(SECOND|MINUTE|HOUR|DAY|WEEK|MONTH|YEAR)$/i.test(
							value,
						);
						if (isIntervalValue) {
							escapedValue = value;
...
app.use(
	"*",
	cors({
		origin: ["https://studio.localflare.dev", "http://localhost:5173", "http://localhost:5174"],
		credentials: true,
	}),
);
