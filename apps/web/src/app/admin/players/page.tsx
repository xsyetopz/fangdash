...
function RoleBadge({ role }: { role: string | null }) {
  if (role === "admin") return <span className="rounded-full bg-orange-500/20 px-2 py-0.5 text-xs font-bold text-orange-400">admin</span>;
  if (role === "dev") return <span className="rounded-full bg-purple-500/20 px-2 py-0.5 text-xs font-bold text-purple-400">dev</span>;
  return <span className="rounded-full bg-white/10 px-2 py-0.5 text-xs font-bold text-gray-400">user</span>;
}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/10">
              <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-widest text-gray-500">Name</th>
              <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-widest text-gray-500">Role</th>
              <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-widest text-gray-500">Status</th>
              <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-widest text-gray-500">Games</th>
              <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-widest text-gray-500">Distance</th>
              <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-widest text-gray-500">Joined</th>
            ...
            {isPending ? (
              ...
              <tr key={p.id} className="transition hover:bg-white/5">
                ...
                <td className="px-4 py-3">
                  {p.banned
                    ? <span className="rounded-full bg-red-500/10 px-2 py-0.5 text-xs font-bold text-red-400">Banned</span>
                ...
                <td className="px-4 py-3 font-mono text-gray-300">{((p.totalDistance ?? 0) / 1000).toFixed(1)} km</td>
      ...
      {banModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0a1628] p-6">
            ...
            <div className="mb-4">
              <label className="mb-1 block text-xs font-bold uppercase tracking-widest text-gray-500">Reason</label>
              <input
                type="text"
                value={banReason}
            ...
            <div className="mb-6">
              <label className="mb-1 block text-xs font-bold uppercase tracking-widest text-gray-500">Duration</label>
              <select
                value={banDuration}
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => banMutation.mutate({ userId: banModal.userId, reason: banReason || undefined, durationDays: banDuration })}
